...
(function($,window){
  ...
  var quickFindApp = $('#myApp')[0]; //ik: get app in DOM
  ...
  function show(pageName,param) {
    ...
    if(currentPageName) { // "close" current page view
      $('#quickFindApp').removeClass(currentPageName); // remove old page class {
      if($currentPage) {
    ...
    $('#quickFindApp').addClass(currentPageName = pageName); // set new page class
    if($currentPage = $page)
  ...
  function app(pageName,param) {
    ...
    var $page = $('#quickFindApp').find("section#" + pageName);
